...
class PowerGrid:
   ...
   def optimize_connections(self) -> List[Tuple[str, str, float]]:
       ...
       visited = set()
       mst_edges = []
       edge_queue = [(0, None, start_station)]

       for neighbor, cost in self.stations[start_station].connections.items():
           heapq.heappush(edge_queue, (cost, start_station, neighbor))

       while edge_queue and len(visited) < len(self.stations):
           cost, current, next_station = heapq.heappop(edge_queue)
           ...
           if current is not None:
               mst_edges.append((current, next_station, cost))

           for neighbor, new_cost in self.stations[next_station].connections.items():
               if neighbor not in visited:
                   heapq.heappush(edge_queue, (new_cost, next_station, neighbor))

       return mst_edges

   def get_total_cost(self, mst: List[Tuple[str, str, float]]) -> float:
      """Calculate total cost of power line installation"""
      total_cost = sum(cost for cost in mst)
      return total_cost

   def display_network(self, mst: List[Tuple[str, str, float]]):
      """Display the optimized power grid network"""
      print("Optimized Power Grid Network:")
      for station1, station2, cost in mst:
          print(f"Connection: {station1} <-> {station2} | Cost: €{cost:,.0f}")
